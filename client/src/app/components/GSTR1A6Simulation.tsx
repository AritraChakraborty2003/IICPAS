...
interface GSTR1A6SimulationProps {
  onComplete?: () => void;
}

// /simulations/gst/gstr-1a-6 -> gst-gstr-1a-6 (matches the slug derivation
...
const SIMULATION_SLUG = "gst-gstr-1a-6";
...
const RATE_OPTIONS = ["0%", "0.1%", "0.25%", "1%", "1.5%", "3%", "5%", "6%", "7.5%", "12%", "18%", "28%"];

const returnsCalendarStatuses = ["Filed", "Filed", "Filed", "Filed", "To be Filed"];

// Up to this many state-wise B2C(Others) transactions can be configured for
// this exercise, via numbered "State N" / "Rate N" / "Taxable Value N"
// credential fields in the Simulation Manager (N = 1..MAX_ROWS). Rows with
// any field left blank are ignored, so admins only need to add as many
// numbered field triples as transactions this exercise requires.
const MAX_ROWS = 8;

const stateNameFromPos = (pos: string) => pos.replace(/^\d+-/, "").trim();
const posOptionForState = (state: string) =>
  POS_OPTIONS.find(
    (opt) => stateNameFromPos(opt).toLowerCase() === state.trim().toLowerCase()
  ) || "Select";
...
export default function GSTR1A6Simulation({
  onComplete,
}: GSTR1A6SimulationProps = {}) {
  ...
  // Admin-configured (Simulation Manager / Course editor) list of state-wise
  // B2C (Others) transactions for this exercise. Never rendered in a banner
  // — only used to validate what the student enters into the form. There is
  // no local fallback: the answer key comes entirely from the numbered
  // "State N" / "Rate N" / "Taxable Value N" credential fields set for this
  // slug in the Simulation Manager.
  const expectedRows: ExpectedRow[] = [];
  for (let i = 1; i <= MAX_ROWS; i++) {
    const numbered = (base: RegExp) =>
      new RegExp(`${base.source}\\D*${i}(?!\\d)`, "i");
    const state =
      findFieldValue(simConfig, numbered(/state/i)) ||
      (i === 1 ? findFieldValue(simConfig, /^state$/i) : "");
    const rate =
      findFieldValue(simConfig, numbered(/rate/i)) ||
      (i === 1 ? findFieldValue(simConfig, /^rate$/i) : "");
    const taxableValue =
      findFieldValue(simConfig, numbered(/taxable/i)) ||
      (i === 1 ? findFieldValue(simConfig, /^taxable.*value$/i) : "");
    if (state && rate && taxableValue) {
      expectedRows.push({ state, rate, taxableValue });
    }
  }
  const isExerciseConfigured = expectedRows.length > 0;

  const [isExperimentStarted, setIsExperimentStarted] = useState(false);
  // This exercise is only about the B2C (Others) add-invoice screen, so the
  ...
  // B2C (Others) - Add Details form state
  ...
  const [addedRows, setAddedRows] = useState<
    { pos: string; rate: string; taxableValue: string; cess: string }[]
  >([]);

  const remainingCount = expectedRows.length - matchedIndices.size;
  const supplyType =
    pos === "Select"
      ? ""
  ...
  const handleFillDetail = () => {
    const nextIdx = expectedRows.findIndex((_, idx) => !matchedIndices.has(idx));
    if (nextIdx === -1) return;
    const next = expectedRows[nextIdx];
    setSaveError("");
    setPos(posOptionForState(next.state));
    setRate(next.rate.includes("%") ? next.rate : `${next.rate}%`);
    setTaxableValue(next.taxableValue);
    setCess("");
  };

  const rowFormComplete = pos !== "Select" && rate !== "Select" && taxableValue.trim();

  const handleSaveInvoice = () => {
    if (!isExerciseConfigured) {
      setSaveError(
        "This exercise hasn't been configured yet. Ask your admin to set the state-wise B2C transactions (State / Rate / Taxable Value) for this simulation in the Simulation Manager."
      );
      return;
    }
    if (!rowFormComplete) {
    ...
    const enteredState = stateNameFromPos(pos);
    const matchIdx = expectedRows.findIndex(
      (row, idx) =>
        !matchedIndices.has(idx) &&
        row.state.trim().toLowerCase() === enteredState.toLowerCase() &&
        normalizeRate(row.rate) === normalizeRate(rate) &&
        normalizeAmount(row.taxableValue) === normalizeAmount(taxableValue)
    );

    if (matchIdx === -1) {
      setSaveError(
        "This transaction doesn't match any expected entry for this exercise. Please double-check the State, Rate, and Taxable Value."
      );
      setSavedMessage("");
      return;
    }

    const nextMatched = new Set(matchedIndices).add(matchIdx);
    setMatchedIndices(nextMatched);
    setAddedRows((prev) => [...prev, { pos, rate, taxableValue, cess }]);
    setSaveError("");

    const stillRemaining = expectedRows.length - nextMatched.size;
    if (stillRemaining === 0) {
      setSavedMessage("");
      ...
      setSavedMessage(
        `Saved. ${nextMatched.size} of ${expectedRows.length} transactions entered.`
      );
    }
    setPos("Select");
    setTaxableValue("");
              ...
              {idx === steps.length - 1 ? (
                ...
                <span
                  ...
                  onClick={() => {
                    ...
                    if (label === "B2CS") setStep("b2c_others_add");
                  }}
                >
                  {label}
                </span>
  ...
  return (
    <div className="min-h-screen bg-white text-[#333333] font-sans antialiased flex flex-col select-none relative">
            ...
            <div className="bg-white border border-[#cbd5e1]">
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {recordCards.map((card) => {
                  const isClickable = card.key === "b2c_others";
                  const count = card.key === "b2c_others" ? addedRows.length : 0;
                  return (
                    <div
                      key={card.key}
                      onClick={isClickable ? () => setStep("b2c_others_add") : undefined}
      ...
      {/* STEP 4: B2C (Others) - Add Details form */}
      {step === "b2c_others_add" && (
        <div className="flex-1 w-full bg-[#f1f5f9] flex flex-col">
          {navHeader}
          {breadcrumb(["Dashboard", "Returns", "GSTR-1/IFF", "B2CS"])}
            ...
            <div className="bg-white border border-[#cbd5e1] p-5 text-[11px]">
              <div className="flex items-center justify-between mb-4">
                <button
                  ...
                  disabled={!isExerciseConfigured || remainingCount === 0}
                  title={
                  ...
                  className={`inline-flex items-center gap-1.5 font-bold uppercase text-[10.5px] px-3.5 py-2 rounded transition-colors ${
                    isExerciseConfigured && remainingCount > 0
              ...
              {isExerciseConfigured && (
                <div className="mb-4 text-[10.5px] font-bold text-slate-500">
                  {remainingCount > 0
                    ? `${addedRows.length} of ${expectedRows.length} transactions entered.`
                    : `All ${expectedRows.length} transactions entered.`}
                </div>
              )}
              ...
              <div className="flex items-start gap-2 mb-4 text-[10.5px] text-slate-600">
              ...
              {savedMessage && (
                <div className="rounded border border-emerald-300 bg-emerald-50 px-2.5 py-2 text-[11px] text-emerald-700 mb-3">
                  {savedMessage}
                </div>
              )}
