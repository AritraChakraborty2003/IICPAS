...
export default function ExperimentLaunchScreen({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-[#08111f] px-4 py-6 text-white">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.12),transparent_26%),radial-gradient(circle_at_top_right,rgba(56,189,248,0.18),transparent_22%),linear-gradient(135deg,#07111f_0%,#0b1b33_45%,#08111f_100%)]" />
      <div className={`absolute -left-16 top-8 h-80 w-80 rounded-full ${tone.glow} blur-3xl`} />
      <div className={`absolute -right-20 bottom-10 h-[26rem] w-[26rem] rounded-full ${tone.glow} blur-3xl`} />
      ...
      <div className="relative z-10 h-full w-full">
        ...
        <button
          onClick={onStart}
          className={`absolute left-1/2 top-1/2 inline-flex min-h-[84px] w-[min(92vw,40rem)] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[22px] px-8 text-2xl font-black uppercase tracking-[0.14em] text-white transition-transform duration-200 hover:scale-[1.02] sm:px-10 sm:text-3xl ${tone.button}`}
