...
type Step = "home" | "login" | "dashboard";

interface ITRReg10SimulationProps {
  onComplete?: () => void;
}

// /simulations/itr-reg-10 -> itr-reg-10 (matches the slug derivation used by
...
const SIMULATION_SLUG = "itr-reg-10";

const DEFAULT_USERNAME = "AKSPA3663B";
...
export default function ITRReg10Simulation({ onComplete }: ITRReg10SimulationProps = {}) {
  const simConfig = useSimulationConfig(SIMULATION_SLUG);
  const username = findFieldValue(simConfig, /user/i) || DEFAULT_USERNAME;
  ...
  // Successful login completes this exercise immediately - the exercise is
  // just about reaching the logged-in dashboard, not filing the return.
  const handleLogin = () => {
    ...
    if (
      ...
      (usernameInput.trim().toUpperCase() !== username.toUpperCase() ||
        passwordInput !== password)
    ) {
      setLoginError("Invalid Username or Password. Please use the credentials provided for this experiment.");
      return;
    }
    setLoginError("");
    setStep("dashboard");
    ...
    <div className="min-h-screen bg-white text-[#333333] font-sans antialiased flex flex-col select-none relative">
      ...
      {showSuccessOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[1.5px] animate-fadeIn">
          <div className="flex flex-col items-center gap-5">
            ...
            <p className="text-white font-bold text-[15px]">Login Successful!</p>
      ...
      <div className="w-full select-none shrink-0 border-b border-slate-200">
        <div className="px-5 py-3 flex items-center justify-between gap-4">
          ...
          {step === "dashboard" ? (
            ...
            <div className="flex items-center gap-4 text-slate-600">
              ...
              <button
                onClick={goToLogin}
                className="border-2 border-red-500 bg-[#0f3a9a] text-white text-[11px] font-bold px-4 py-2 rounded cursor-pointer hover:bg-[#0a2558]"
              >
                Login
              </button>
        ...
        <div className="bg-[#1e3b6a] px-4 text-[11px] font-bold flex flex-wrap items-center text-white shadow-md">
          {(step === "dashboard" ? dashboardNavItems : navItems).map((item) => (
            <span
              key={item}
      ...
      <div className="flex-1 w-full flex flex-col">
        ...
        {step === "login" && (
          <div className="flex-1 w-full px-6 py-10 flex justify-center">
            <div className="w-full max-w-sm border border-slate-200 rounded p-6 shadow-sm">
              <h3 className="text-[22px] font-bold text-[#0a2558] mb-5">Login</h3>
              ...
              <input
                ...
                className="w-full border border-slate-300 rounded px-2.5 py-2 text-[12px] font-semibold uppercase text-slate-800 outline-none focus:border-blue-500 mb-3"
              />
              ...
              <div className="border border-slate-200 rounded p-4">
                ...
                <div className="text-[11.5px] space-y-2 pt-2 border-t border-slate-100">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[#0f3a9a] font-bold cursor-default">Secure Account</span>
                  </div>
                </div>
              </div>
