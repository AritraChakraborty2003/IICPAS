...
import { CheckCircle2, RotateCcw, RefreshCw, Wand2, X } from "lucide-react";
...
interface GSTR1A8SimulationProps {
  onComplete?: () => void;
}

// /simulations/gst/gstr-1a-8 -> gst-gstr-1a-8 (matches the slug derivation
...
const SIMULATION_SLUG = "gst-gstr-1a-8";
...
const DOC_TYPE_OPTIONS = ["Debit Note", "Credit Note"];

const NOTE_REASON_OPTIONS = [
  "Sales Return",
  "Post Sale Discount",
...
// Rate rows shown in the Item details table, exactly as listed on the GST
// portal's Credit/Debit Notes (Unregistered) - Add Note screen.
const ITEM_RATE_ROWS = [
  "0%",
  "0.1%",
  "0.25%",
  "1%",
...
const normalizeRate = (rate: string) => rate.replace(/[%\s]/g, "");
...
// Resolve an admin-configured rate value (e.g. "18", "18%") to one of the
// Item details table's rate row labels.
const findRateRow = (value: string) => {
  const target = normalizeRate(value);
  return ITEM_RATE_ROWS.find((label) => normalizeRate(label) === target) || "";
};
...
export default function GSTR1A8Simulation({
  onComplete,
}: GSTR1A8SimulationProps = {}) {
  ...
  // the Credit/Debit Notes (Unregistered) - Add Note exercise. Never
  // rendered in a banner — only used to validate what the student types into
  // the form. There is no local fallback: this exercise's answer key comes
  // entirely from the "Type" / "Note No." / "Note Date" / "Note Value" /
  // "POS" / "Rate" / "Taxable Value" credential fields set for this slug in
  // the Simulation Manager.
  const expectedDocType = findFieldValue(simConfig, /^type$/i) || findFieldValue(simConfig, /debit.*credit.*type/i);
  ...
  const expectedRate = findFieldValue(simConfig, /rate/i);
  ...
  // This exercise is only about the CDNUR add-note screen, so the simulation
  ...
  // Credit/Debit Notes (Unregistered) - Add Note form state
  const [docType, setDocType] = useState("Select");
  const [noteNo, setNoteNo] = useState("");
  const [noteDate, setNoteDate] = useState("");
  const [noteValue, setNoteValue] = useState("");
  const [noteReason, setNoteReason] = useState("Select");
  const [pos, setPos] = useState("Select");
  const [itemTaxableValues, setItemTaxableValues] = useState<Record<string, string>>({});
  ...
  const rateNumber = (label: string) => parseFloat(label.replace("%", ""));
  const computeIntegratedTax = (label: string, taxable: string) => {
    const t = parseFloat(taxable);
    if (!taxable.trim() || isNaN(t)) return "";
    const tax = (t * rateNumber(label)) / 100;
    return Number.isInteger(tax) ? String(tax) : tax.toFixed(2);
  };

  const handleItemTaxableChange = (label: string, value: string) => {
    setSaveError("");
    setItemTaxableValues((prev) => ({ ...prev, [label]: value }));
  };
  ...
  const resetNoteForm = () => {
    ...
    setNoteDate("");
    setNoteValue("");
    setNoteReason("Select");
    setPos("Select");
    setItemTaxableValues({});
    setSaveError("");
  };
  ...
  const handleFillDetail = () => {
    ...
    setSaveError("");
    setDocType(
      DOC_TYPE_OPTIONS.find((opt) => normalizeDocType(opt) === normalizeDocType(expectedDocType)) ||
        "Select"
    );
    ...
    const rateLabel = findRateRow(expectedRate);
    setItemTaxableValues(rateLabel ? { [rateLabel]: expectedTaxableValue } : {});
  };

  const filledItemRows = ITEM_RATE_ROWS.filter((label) => itemTaxableValues[label]?.trim());
  ...
  const formComplete =
    ...
    noteReason !== "Select" &&
    pos !== "Select" &&
    filledItemRows.length === 1;

  const handleSaveNote = () => {
    if (!isExerciseConfigured) {
      setSaveError(
        "This exercise hasn't been configured yet. Ask your admin to set the Type, Note No., Note Date, Note Value, POS, Rate and Taxable Value for this simulation in the Simulation Manager."
      );
      return;
    }
    if (filledItemRows.length > 1) {
      setSaveError("Please enter the Taxable value against only one applicable GST rate.");
      return;
    }
    if (!formComplete) {
      setSaveError("Please fill all mandatory fields, including the Taxable value against the applicable GST rate.");
      return;
    }

    const rateLabel = filledItemRows[0];
    const taxableValue = itemTaxableValues[rateLabel];
    const integratedTax = computeIntegratedTax(rateLabel, taxableValue);

    const isValid =
      normalizeDocType(docType) === normalizeDocType(expectedDocType) &&
      ...
      normalizeRate(rateLabel) === normalizeRate(expectedRate) &&
      normalizeAmount(taxableValue) === normalizeAmount(expectedTaxableValue);

    if (!isValid) {
      setSaveError(
        "The credit/debit note details entered do not match this exercise. Please double-check the Type, Note No., Note Date, Note Value, POS and the Taxable value entered against the correct GST rate."
      );
      return;
    }
              ...
              {idx === steps.length - 1 ? (
                ...
                <span
                  ...
                  onClick={() => {
                    ...
                    if (label === "CDNUR") setStep("cdnur_summary");
                  }}
                >
                  {label}
                </span>
  ...
  return (
    <div className="min-h-screen bg-white text-[#333333] font-sans antialiased flex flex-col select-none relative">
            ...
            <div className="bg-white border border-[#cbd5e1]">
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {recordCards.map((card) => {
                  const isClickable = card.key === "cdnur";
                  const count = card.key === "cdnur" && addedNote ? 1 : 0;
                  return (
                    <div
                      key={card.key}
                      onClick={isClickable ? () => setStep("cdnur_summary") : undefined}
      ...
      {/* STEP 4: CDNUR Summary */}
      {step === "cdnur_summary" && (
        <div className="flex-1 w-full bg-[#f1f5f9] flex flex-col">
          {navHeader}
          {breadcrumb(["Dashboard", "Returns", "GSTR-1", "CDNUR"])}

          <div className="flex-1 p-5 space-y-4 animate-fadeIn">
            <div className="bg-white border border-[#cbd5e1]">
              <div className="bg-[#0d9488] text-white px-4 py-2.5 flex items-center justify-between font-extrabold text-[12px]">
                <span>Credit / Debit Notes (Unregistered) - Summary</span>
                <div className="flex items-center gap-2">
                  <button className="bg-[#0b7a70] hover:bg-[#0a6a61] text-white text-[9.5px] font-bold uppercase px-3 py-1.5">
                    Help
                  </button>
              ...
              <div className="p-5">
                {!addedNote ? (
                  <div className="bg-[#e0f2fe] border border-[#bae6fd] text-[#0369a1] px-3 py-2.5 rounded text-[11px] mb-4 flex items-center justify-between">
                    ...
                    <X size={13} className="text-[#0369a1] cursor-pointer" />
                  </div>
                ) : (
                  <>
                      ...
                      <table className="w-full text-[10.5px]">
                        <thead>
                          <tr className="bg-slate-100 text-slate-700 font-extrabold">
                            ...
                            <th className="border border-slate-300 px-3 py-2 text-left">POS</th>
                            <th className="border border-slate-300 px-3 py-2 text-left">Taxable Value (₹)</th>
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td className="border border-slate-300 px-3 py-1.5 text-slate-700 font-semibold">
                              {stateNameFromPos(addedNote.pos)}
                            </td>
                            <td className="border border-slate-300 px-3 py-1.5 text-slate-700 font-semibold">
                              {addedNote.taxableValue}
                            </td>
                            <td className="border border-slate-300 px-3 py-1.5 text-slate-700 font-semibold">
                              {addedNote.noteValue}
                            </td>
        ...
        <div className="flex-1 w-full bg-[#f1f5f9] flex flex-col">
          ...
          <div className="flex-1 p-5 space-y-4 animate-fadeIn">
            <div className="bg-[#0d9488] text-white px-4 py-2.5 font-extrabold text-[12px]">
              Credit/Debit Notes (Unregistered)- Add Note
            </div>
            ...
            <div className="bg-white border border-[#cbd5e1] p-5 text-[11px]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setSaveError("");
                      setNoteReason(e.target.value);
                    }}
                    className="w-full border border-slate-300 rounded px-2.5 py-1.5 bg-white text-slate-800 text-[11px] outline-none font-semibold"
                  >
                    <option value="Select">Select</option>
                    {NOTE_REASON_OPTIONS.map((opt) => (
                      <option key={opt} value={opt}>
                        {opt}
                      </option>
                    ))}
              ...
              <h3 className="font-extrabold text-[#0a2558] text-[12px] mb-3">Item details</h3>

              <div className="overflow-x-auto border border-slate-300 mb-4">
                <table className="w-full text-[10.5px]">
                  <thead>
                    <tr className="bg-slate-100 text-slate-700 font-extrabold">
                      <th rowSpan={2} className="border border-slate-300 px-3 py-2 text-left align-bottom">
                        Rate (%)
                      </th>
                      <th rowSpan={2} className="border border-slate-300 px-3 py-2 text-left align-bottom">
                    ...
                    {ITEM_RATE_ROWS.map((label) => (
                      <tr key={label}>
                        ...
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            value={itemTaxableValues[label] || ""}
                            onChange={(e) => handleItemTaxableChange(label, e.target.value)}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-white text-slate-800 text-[11px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            disabled
                            value={computeIntegratedTax(label, itemTaxableValues[label] || "")}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-slate-100 text-slate-500 text-[11px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
