...
type Step = "prepare" | "summary" | "declare";

interface GSTR1A22SimulationProps {
  onComplete?: () => void;
}

// /simulations/gst/gstr-1a-22 -> gst-gstr-1a-22 (matches the slug derivation
...
export default function GSTR1A22Simulation({
  onComplete,
}: GSTR1A22SimulationProps = {}) {
  ...
  // Clicking "File GSTR-1 Return" in the OTP modal completes the exercise
  // immediately - the JSON upload data is assumed correct per the exercise
  // brief, so there's nothing left to validate beyond the OTP itself.
      ...
      <div className="flex-1 w-full p-5 flex flex-col gap-5 relative">
        ...
        {showSuccessOverlay && (
          <div className="bg-[#dcfce7] border border-[#86efac] text-[#166534] text-[11px] font-semibold px-4 py-3 rounded">
            GSTR1 of GSTIN - {gstin} for the Return Period - &apos;May&apos; - has been
          ...
          {step === "prepare" && (
            <div className="p-5 space-y-4 text-[11px]">
              <div className="bg-[#e0f2fe] border border-[#bae6fd] text-[#0369a1] px-3 py-2.5 rounded">
                Data uploaded through the JSON file generated from TallyPrime has been imported
                successfully. Please generate the GSTR-1 summary to enable Preview (summary PDF)
                and Submit/file.
              </div>
                ...
                <table className="w-full text-[10.5px] border border-slate-200">
                  <thead>
                    <tr className="bg-slate-100 text-slate-600 font-bold">
                      <th className="px-3 py-2 text-left border-b border-slate-200">Description</th>
                      <th className="px-3 py-2 text-right border-b border-slate-200">Value (₹)</th>
                      ...
                      <th className="px-3 py-2 text-right border-b border-slate-200">Cess (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              {step === "declare" && (
                <div className="border-t border-slate-200 pt-4">
                  <label className="flex items-start gap-2 font-semibold text-slate-700">
                    ...
                    is true and correct to the best of my/our knowledge and belief and nothing
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[1.5px] px-4">
          <div className="w-full max-w-sm rounded bg-white p-5 shadow-2xl">
            ...
            <div className="bg-[#ecfdf5] border border-[#86efac] text-[#166534] text-[10.5px] px-3 py-2.5 rounded mb-3">
              One-Time Password (OTP) has been sent to your registered email ID and mobile
              number. OTP is Valid Till 10 mins
            </div>
