...
export default function CertificateTab({
  ...
  // Cross-origin uploads (e.g. api.iicpa.in/uploads/...) are served straight off
  // disk by nginx in production and never get the app's CORS headers, so a
  // direct fetch()/crossOrigin Image() load fails and the canvas draw is
  // silently skipped. Routing through our own same-origin API route sidesteps
  // that: the browser only ever talks to its own origin, and the cross-origin
  // fetch happens server-side where CORS doesn't apply.
  const toCanvasSafeUrl = (src) => {
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="absolute inset-0 pointer-events-none select-none">
                          <div 
                            className="absolute w-full text-center text-slate-600 font-serif italic text-[4.5px] leading-none -translate-y-full"
                            style={{ top: "39.5%" }}
                          >
                            This is to certify that
                          </div>
                          <div 
                            className="absolute w-full text-center font-serif font-bold text-slate-900 uppercase text-[6.5px] leading-none px-2 truncate -translate-y-full"
                            style={{ top: "46.5%" }}
                          >
                            {currentStudentName || "Student Name"}
                          </div>
                          <div 
                            className="absolute text-left font-sans font-bold text-blue-900 text-[4.5px] leading-none -translate-y-full max-w-[15%] truncate"
                          ...
                          {lokeshSignUrl && (
                            <div
                              ...
                              style={{ top: "71.5%", left: "23.9%", width: "11%", height: "7%" }}
                            >
                              <img
                                src={lokeshSignUrl}
                                alt="Signature"
                          ...
                          {poonamSignUrl && (
                            <div
                              className="absolute -translate-x-1/2 -translate-y-full pointer-events-none select-none"
                              style={{ top: "71.5%", left: "76.1%", width: "11%", height: "7%" }}
                            >
                              <img
                                src={poonamSignUrl}
                                alt="Signature"
              ...
              <div className="max-w-4xl w-full">
                <div className="relative aspect-[1.414/1] rounded-2xl overflow-hidden bg-[#1e293b] shadow-2xl border border-white/5 transition-all duration-500">
                  ...
                  <div className="absolute inset-0 pointer-events-none select-none">
                    {/* 'This is to certify that' */}
                    <div 
                      className="absolute w-full text-center text-slate-600 font-serif italic text-[1.6%] leading-none -translate-y-full"
                      style={{ top: "39.5%" }}
                    >
                      This is to certify that
                    </div>
                    ...
                    <div 
                      className="absolute w-full text-center font-serif font-bold text-slate-900 tracking-wide uppercase text-[2.5%] leading-none drop-shadow-sm px-4 -translate-y-full"
                      style={{ top: "46.5%" }}
                    >
                      {currentStudentName || "Student Name"}
                    </div>
                    ...
                    <div 
                      className="absolute text-left font-sans font-bold text-blue-900 text-[1.2%] leading-none tracking-tight -translate-y-full"
