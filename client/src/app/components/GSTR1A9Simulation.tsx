...
interface GSTR1A9SimulationProps {
  onComplete?: () => void;
}

// /simulations/gst/gstr-1a-9 -> gst-gstr-1a-9 (matches the slug derivation
...
const SIMULATION_SLUG = "gst-gstr-1a-9";
...
// Table 13 - Documents issued during the tax period. Only the first category
// ("Invoices for outward supply") is fillable for this exercise; the rest
// mirror the GST portal's empty-state tables.
...
// Reference exercise data (Experiment 9): company issued 100 invoices
// numbered UT24/10251 to FN24/10350, of which 2 were cancelled. These are
// validation-only fallbacks and are overridden by the "Invoice From" /
// "Invoice To" / "Total Number" / "Cancelled" credential fields an admin
// sets for this slug in the Simulation Manager / Course editor.
const DEFAULT_FROM = "UT24/10251";
const DEFAULT_TO = "FN24/10350";
const DEFAULT_TOTAL = "100";
const DEFAULT_CANCELLED = "2";

export default function GSTR1A9Simulation({
  onComplete,
}: GSTR1A9SimulationProps = {}) {
  ...
  // the Documents Issued - Invoices for outward supply row. Never rendered
  // in a banner — only used to validate what the student types into the
  // form.
  const expectedFrom = findFieldValue(simConfig, /invoice.*from|sr.*no.*from|^from$/i) || DEFAULT_FROM;
  const expectedTo = findFieldValue(simConfig, /invoice.*to|sr.*no.*to|^to$/i) || DEFAULT_TO;
  const expectedTotal = findFieldValue(simConfig, /total.*number|total.*doc/i) || DEFAULT_TOTAL;
  const expectedCancelled = findFieldValue(simConfig, /cancel/i) || DEFAULT_CANCELLED;

  const [isExperimentStarted, setIsExperimentStarted] = useState(false);
  // This exercise is only about the Documents Issued screen, so the
  ...
  // Documents Issued - "Invoices for outward supply" row state
  const [docFrom, setDocFrom] = useState("");
  const [docTo, setDocTo] = useState("");
  const [docTotal, setDocTotal] = useState("");
  const [docCancelled, setDocCancelled] = useState("");
  const [saveError, setSaveError] = useState("");

  const docNetIssued =
    docTotal.trim() && docCancelled.trim() && !isNaN(Number(docTotal)) && !isNaN(Number(docCancelled))
      ? String(Number(docTotal) - Number(docCancelled))
      : "";
  ...
  const handleFillDetail = () => {
    setSaveError("");
    setDocFrom(expectedFrom);
    setDocTo(expectedTo);
    setDocTotal(expectedTotal);
    setDocCancelled(expectedCancelled);
  };

  const rowFormComplete =
    docFrom.trim() && docTo.trim() && docTotal.trim() && docCancelled.trim();
  ...
  const handleSaveDocuments = () => {
    ...
    const isValid =
      docFrom.trim().toUpperCase() === expectedFrom.trim().toUpperCase() &&
      docTo.trim().toUpperCase() === expectedTo.trim().toUpperCase() &&
      normalizeAmount(docTotal) === normalizeAmount(expectedTotal) &&
      normalizeAmount(docCancelled) === normalizeAmount(expectedCancelled);

    if (!isValid) {
      setSaveError(
        "The documents issued details entered do not match this exercise. Please double-check the Sr. No. From/To, Total Number and Cancelled fields."
      );
      return;
    }
  ...
  // An empty-state Table 13 category (all categories except "Invoices for
  // outward supply", which is the only one this exercise requires filling).
  const emptyDocTable = (label: string, no: number) => (
    <div key={label} className="mb-6">
        ...
        <table className="w-full text-[10.5px]">
          <thead>
            <tr className="bg-slate-100 text-slate-700 font-extrabold">
              ...
              <th colSpan={2} className="border border-slate-300 px-3 py-1.5 text-center">
              ...
              <th rowSpan={2} className="border border-slate-300 px-3 py-2 text-left align-bottom">
                Net issued<span className="text-red-500">*</span>
              </th>
              <th rowSpan={2} className="border border-slate-300 px-3 py-2 text-left align-bottom">
                Action
              </th>
            ...
            <tr className="bg-slate-100 text-slate-700 font-extrabold">
              <th className="border border-slate-300 px-3 py-1.5 text-center">From</th>
              <th className="border border-slate-300 px-3 py-1.5 text-center">To</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td colSpan={7} className="border border-slate-300 bg-[#e0f2fe] text-[#0369a1] px-3 py-2.5 text-center">
      ...
      <div className="flex justify-end mt-2">
        <button
          disabled
          className="bg-slate-200 text-slate-400 font-bold uppercase text-[10px] px-4 py-1.5 rounded cursor-not-allowed"
        >
          Add Document
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white text-[#333333] font-sans antialiased flex flex-col select-none relative">
            ...
            <div className="bg-white border border-[#cbd5e1]">
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {recordCards.map((card) => {
                  const isClickable = card.key === "documents";
                  const count = card.key === "documents" && savedDocuments ? 1 : 0;
                  return (
                    <div
                      key={card.key}
                      onClick={isClickable ? () => setStep("documents_issued") : undefined}
      ...
      {/* STEP 4: Documents Issued (Table 13) */}
      {step === "documents_issued" && (
        <div className="flex-1 w-full bg-[#f1f5f9] flex flex-col">
          {navHeader}
          {breadcrumb(["Dashboard", "Returns", "GSTR-1/IFF", "Documents Issued"])}

          <div className="flex-1 p-5 space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between">
              <h2 className="text-[#0a2558] font-extrabold text-[14px]">
                13 - Documents issued during the tax period
              </h2>
              <button
                className="bg-[#0f3a9a] hover:bg-[#0a2558] text-white p-1.5"
                aria-label="Refresh"
              >
                <RefreshCw size={12} />
              </button>
            ...
            <div className="bg-white border border-[#cbd5e1] p-5">
              <div className="flex items-center justify-between mb-4">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 font-bold uppercase text-[10.5px] px-3.5 py-2 rounded transition-colors bg-[#0f3a9a] hover:bg-[#0a2558] text-white cursor-pointer"
                >
                  <Wand2 size={12} />
                  Fill Detail
                </button>
                <span className="text-[10.5px] text-slate-600">
                  Note: Kindly click on save button after any modification (add, edit,
                  delete) to save the changes
                </span>
              </div>

              {/* 1. Invoices for outward supply (fillable) */}
              <div className="mb-6">
                <h4 className="font-extrabold text-[#0f3a9a] text-[11.5px] mb-2">
                ...
                <div className="overflow-x-auto border border-slate-300">
                  <table className="w-full text-[10.5px]">
                    ...
                    <tbody>
                      <tr>
                        <td className="border border-slate-300 px-3 py-1.5 text-center text-slate-700 font-semibold">
                          1
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            ...
                            onChange={(e) => {
                              setSaveError("");
                              setDocFrom(e.target.value.toUpperCase());
                            }}
                            className="w-full border border-red-500 rounded px-2 py-1 bg-white text-slate-800 text-[10.5px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            ...
                            onChange={(e) => {
                              setSaveError("");
                              setDocTo(e.target.value.toUpperCase());
                            }}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-white text-slate-800 text-[10.5px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            ...
                            onChange={(e) => {
                              setSaveError("");
                              setDocTotal(e.target.value.replace(/[^\d]/g, ""));
                            }}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-white text-slate-800 text-[10.5px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            ...
                            onChange={(e) => {
                              setSaveError("");
                              setDocCancelled(e.target.value.replace(/[^\d]/g, ""));
                            }}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-white text-slate-800 text-[10.5px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5">
                          <input
                            disabled
                            value={docNetIssued}
                            className="w-full border border-slate-300 rounded px-2 py-1 bg-slate-100 text-slate-500 text-[10.5px] outline-none font-semibold"
                          />
                        </td>
                        <td className="border border-slate-300 px-2 py-1.5 text-center">
                          <button
                            ...
                            className="bg-[#e1141a] hover:bg-[#c90f15] text-white p-1.5 rounded cursor-pointer"
                          >
                            <FaTrash size={10} />
                          </button>
                        </td>
              ...
              {/* 2-12. Remaining categories (empty state) */}
              {documentCategories.slice(1).map((label, idx) => emptyDocTable(label, idx + 2))}
