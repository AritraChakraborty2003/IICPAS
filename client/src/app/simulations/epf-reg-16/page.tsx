...
// "epf-reg-16", matched by field label: UAN / Password / Name. If nothing
...
const SIMULATION_SLUG = "epf-reg-16";
...
// ─── Post-login top nav ─────────────────────────────────────────────────────
function LoggedInNav({ name }: { name: string }) {
  const [calcOpen, setCalcOpen] = useState(false);
  const tabs = ["Home", "Profile", "Passbook", "Claims", "Service History"];
  return (
    <nav className="relative flex items-center justify-between bg-[#163b71] px-6 text-[13.5px] font-semibold text-white">
      <div className="flex items-stretch">
        {tabs.map((tab, i) => (
          <span
            key={tab}
            className={`cursor-pointer px-4 py-3 ${i === 0 ? "bg-[#0f2a54]" : "hover:bg-white/10"}`}
          >
            {tab}
          </span>
        ))}
...
// ─── Root page ──────────────────────────────────────────────────────────────
export default function EpfReg16Page() {
  const [launched, setLaunched] = useState(false);
  const [view, setView] = useState<View>("login");
  const [showTick, setShowTick] = useState(false);

  // Admin-configured experiment values (Simulation Manager slug
  // "epf-reg-16", or the course editor's per-insert "Add/Edit Creds")
