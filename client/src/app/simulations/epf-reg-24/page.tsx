...
// Creds") for slug "epf-reg-24" — nothing is hardcoded here. The
...
const SIMULATION_SLUG = "epf-reg-24";
    ...
    <div className="fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-[#07111f]/45 px-4 pt-24 backdrop-blur-[3px] sm:pt-32">
      <div className="flex flex-col items-center gap-4 rounded-[16px] bg-white/95 px-10 py-8 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
        <div
          ...
          style={{ animation: "epfReg24TickPop 0.15s ease-out" }}
        >
...
// ─── Root page ──────────────────────────────────────────────────────────────
export default function EpfReg24Page() {
  const [launched, setLaunched] = useState(false);
  const [loggedIn, setLoggedIn] = useState(false);

  // Admin-configured credentials (Simulation Manager slug "epf-reg-24", or the
