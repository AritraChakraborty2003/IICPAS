...
// the admin Simulation Manager (or the course editor's per-insert
// "Add/Edit Creds") for slug "epf-reg-14", matched by field label:
// UAN / Old Password (or Temp Password) / New Password / OTP. If nothing
// is configured, the values below (matching the standard "First time Login
// to Change Password" experiment for Mr. Nagarjun Patel) are used as the
// default expected answers.
const SIMULATION_SLUG = "epf-reg-14";
const DEFAULT_UAN = "201973667382";
const DEFAULT_TEMP_PASSWORD = "cwwHSM$&#921";
const DEFAULT_NEW_PASSWORD = "Fin@123";
const DEFAULT_OTP = "789012";
const CAPTCHA_CODE = "P237M";
const MOBILE_LAST4 = "8777";
...
// ─── Member Sign In panel (first-time login with the temporary SMS
// password) ──────────────────────────────────────────────────────────────
function MemberSignInPanel({
  ...
  const submit = () => {
    ...
    if (
      validateCreds &&
      (normalize(form.uan) !== normalize(uanValue) || form.password !== tempPasswordValue)
    ) {
      setError("Invalid credentials. Please use the UAN and temporary password sent to your mobile.");
      return;
    }
...
function ChangePasswordPage({
  ...
  const submit = () => {
    ...
    if (validateCreds && form.oldPassword !== tempPasswordValue) {
      setError("Old Password does not match the temporary password sent to your mobile.");
      return;
    }
    ...
    if (validateCreds && newPasswordValue && form.newPassword !== newPasswordValue) {
      setError(`Please set the new password given for this experiment (${newPasswordValue}).`);
      return;
    }
    if (!consent) {
...
// ─── Final success page (password changed) ─────────────────────────────────
  ...
  return (
    <main className="mx-auto flex w-[98vw] flex-1 flex-col gap-4 py-6">
      ...
      <div className="flex items-start gap-3 rounded bg-[#e3f3e1] px-4 py-3 text-[14px] text-[#1a7a3a]">
        ...
        <span>
          Your password has been changed successfully. Kindly use your new password the next time you log in to
          your UAN account.
        </span>
      </div>
...
export default function EpfReg14Page() {
  ...
  const tempPasswordValue = findFieldValue(simConfig, /old|temp/i) || DEFAULT_TEMP_PASSWORD;
  const newPasswordValue = findFieldValue(simConfig, /new/i) || DEFAULT_NEW_PASSWORD;
